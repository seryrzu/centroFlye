...
class MonoString:
    ...
    def check_reverse(self):
        ...
        perc_lower_case = np.mean([c >= self.rev_monomer_index
...
class SD_Report:
    def __init__(self, SD_report_fn, monomers_fn,
        ...
        self.monomer_names_map['None'] = none_symb  # '?' is reserved for '*'
        ...
        df.sec_monomer = df.sec_monomer.apply(
            lambda x: self.monomer_names_map[x])
        self.rev_monomer_index = len(self.monomers)

        for r_id, group in df.groupby('r_id'):
            group = group.sort_values(by=['r_st'])
            self.monostrings[r_id] = MonoString.FromSDRecord(
                name=r_id, monomers=group.monomer.to_list(),
                starts=group.r_st.to_list(),
                ends=group.r_en.to_list(),
                reliability=group.reliability.to_list(),
                max_gap=max_gap, mean_monomer_len=mean_monomer_len,
                gap_symb=self.gap_symb,
                rev_monomer_index=self.rev_monomer_index)
        self.df = df

    def get_monomer_encoded_names(self):
        monomer_names = list(self.monomer_names_map.values())
        monomer_names = filter(lambda x: isinstance(x, int) and x < self.rev_monomer_index,
                               monomer_names)
        monomer_names = list(monomer_names)
        return monomer_names

    def is_lower(self, index):
        return index < self.rev_monomer_index

    def is_upper(self, index):
        return not self.is_lower(index)

    def get_rev_index(self, indexes):
        rev_indexes = []
        for index in indexes:
            if self.is_lower(index):
                rev_indexes.append(index + self.rev_monomer_index)
            else:
                rev_indexes.append(index - self.rev_monomer_index)
        return rev_indexes
